...
public:
  ...
  decision_proceduret::resultt loop(const problemt &problem, learnt &learn,
                                   verifyt &verify) {
  ...
  neighbourhoodt &neighbourhood)
{
  ...
  if(!problem.literals.empty() && !neighbourhood.literals_search_done)
  {
    ...
    const synth_encoding_factoryt synth_encoding([this, &problem]()
    {
      std::unique_ptr<synth_encodingt> result(
        new local_synth_encodingt(ns, current_solution, current_constraints));
      result->literals = problem.literals;
      return result;
    });
...
std::unique_ptr<learnt> local_cegist::create_learner()
{
  synth_encoding_factoryt synth_encoding_factory(
    local_synth_encoding(ns, current_solution, current_constraints));
  return create_learner(std::move(synth_encoding_factory));
}
  ...
  const synth_encoding_factoryt synth_encoding_factory)
{
  ...
  if(incremental_solving)
  {
    return std::unique_ptr<learnt>(new incremental_solver_learnt(
      ns, problem, use_simp_solver, msg, synth_encoding_factory));
  }
  else
  {
    std::unique_ptr<solver_learnt> learn(
      new solver_learnt(ns, problem, msg, synth_encoding_factory));
  ...
  const size_t program_size)
{
  ...
  neighbourhood.learn = create_learner();
