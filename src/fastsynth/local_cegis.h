...
public:
  ...
  /// Create a CEGIS default learner instance using the given configuration.
  /// \return Configured CEGIS learner.
  std::unique_ptr<learnt> create_learner();

  /// Create a CEGIS learner of the given type.
  /// \param synth_encoding_factory Learner type to instantiate.
  /// \return Configured CEGIS learner.
  std::unique_ptr<learnt>
  create_learner(synth_encoding_factoryt synth_encoding_factory);
